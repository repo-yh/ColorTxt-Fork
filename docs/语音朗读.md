...
| **AI 识别** | 多音色 + **AI 阅读助手已启用** 时可开 **`aiSpeakerRecognitionEnabled`**：朗读前主进程 **`attributeVoiceReadSpeakers`** **按行**识别说话人姓名、性别与情绪（输入为当前行原文 + 角色表姓名/别名 + **邻近参考行**：向两侧穿过纯对白行至首个非纯对白行后再多收一行，中间纯对白保留，单向最多约 15 非空行；**不**做全书向量检索；结果缓存于 **`voiceReadSpeakerCache.ts`**，缓存键含上下文）；匹配角色 **`voiceReadVoiceId`** 或按男女声回退；消耗对话 Token（可展示在设置/侧栏） |
