...
| 找书设置相关 localStorage | `colortxt.findBook.settings`：缓存/下载目录、网络代理、找书阅读器侧栏等；**阅读 / 编辑 / 语音朗读 / 词典**与主界面共用 `colorTxt.ui.settings` |
...
- 找书设置标签：「下载」「阅读」「编辑」「语音朗读」「代理」（网络代理，见 §6.11）。主界面「设置 → 代理」与找书「代理」**共用**同一份配置。「阅读 / 编辑 / 语音朗读」与主界面共用 `colorTxt.ui.settings`（各窗内存独立，打开窗口时读最后保存值）；编辑页隐藏「自动刷新章节列表」与「AI 智能排版」。
...
- 阅读工具栏：支持「编辑模式」与「保存」（写入当前章 `book_cache` 缓存文件；无「智能排版」）。内存中始终保留原文；**编辑模式直接编辑原文**（不跑文本替换/转换）。阅读模式右键 / **`Ctrl+E`**：**编辑选中文本**（≤1000 字；按展示管线物理行改写并写回章节缓存，再刷新展示）
  ；阅读态经展示管线再渲染。
