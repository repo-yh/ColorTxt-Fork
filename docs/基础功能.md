...
- **段间距（`lineSpacingPx`）**：默认 `0`（关闭），范围 `0`～`100`（px）。加在每个**物理行**（Monaco model line）最后一条 view 行之后；软换行中间不加距。与「压缩空行时保留一个空行」并存（前者为布局，后者为文本管线）。
...
- 主题、侧栏展开、语法着色；**压缩空行** / **章节标题上下保留空行** / **保留一个空行** / **行首缩进**；**`textConvertZh` / `textConvertLetter` / `textConvertDigit`**（顶栏「转换」，默认 **`off`**）；**`timedScroll`**（定时滚动范围与间隔，见 **`constants/timedScroll.ts`**）；**`readerEditShowLineNumbers`**、**`readerEditMinimap`**、**`editAutoRefreshChapterList`**；章节字数；字号与行间距倍数、**`lineSpacingPx`**（段间距，默认 **0**）、**`letterSpacingPx`**（字间距，默认 **0**）；启动恢复会话；Monaco 高级换行；**`monacoCjkWrapOptimize`**（设置 → 阅读「启用中文换行优化」，默认 **开启**：简单换行下将 `“”`/`——`/`……` 等按全角估算并用汉字校准全角字宽；开启高级换行时运行时停用）；**滚动**（**`monacoSmoothScrolling`**、**`mouseWheelScrollSensitivity`** 默认 1、**`fastScrollSensitivity`** 默认 5 / 按住 Alt）；**`stickyChapterTitleEnabled`**（启用粘性章节标题，默认 **开启**）等。
...
- **查找**：主工具条「记笔记」后、「问 AI」前（需设置开启）；**关闭**工具条后按 **`findTarget`** 处理选区文案：**`findBar`**（默认）→ **`ReaderMain.openFindWithSearchString`**；**`sidebarSearch`** → 展开侧栏「搜索」并填入查询。找书窗口无全文搜索侧栏：**不展示**该配置项，但若主界面已设为「全文搜索」，找书内点击「查找」仍**回退查找栏**。
- **词典**：主工具条「查找」后、「问 AI」前（需设置开启，默认不显示）；点击后**关闭**工具条并打开 **`ReaderDictionaryPopup`**（见 **「词典查词」**）。
...
- **工具条显示项**（**设置 → 阅读 → 工具条**）：`selectionToolbarButtons`（`copy` / `find` / **`dictionary`** / `askAi` / **`findTarget`**）控制选区工具条按钮显示与「查找」应用目标（查找栏 / 全文搜索，默认查找栏）。主界面设置页以工具条预览 + 左上角复选框，以及与定时滚动「范围」同款的 **`RadioGroup`**；找书设置 **`showFindTargetOption=false`** 隐藏应用目标。
...
- **选区工具条「词典」**（**只读**）：需 **`selectionToolbarButtons.dictionary`**（默认关）；点击后关闭工具条，打开 **`ReaderDictionaryPopup.vue`**。
...
- **本地词库**：导入后写入 **`localCacheDir`**（空则默认 **`userData/dictionaries`**，见 **`utils/defaultCacheDirs.ts`**）；支持 StarDict / MDict（`.mdx`，依赖 **`mdict-js`**）/ DICT / Slob / BGL；新导入项插入列表**顶部**。
...
5. MDX 释义经 **`simplifyMdictHtml`** 去掉 script/style 与尾部 **`\0`**。

### 浮层交互（`ReaderDictionaryPopup`）

- 顶栏：改词输入 +「查询」+ 管理齿轮；点外部 / Esc 关闭；无关闭按钮。
