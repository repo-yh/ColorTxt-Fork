...
- **拖放**：见 **`useAppWindowBindings.ts`**：侧栏列表区域合并进文件列表；**非侧栏**区域打开拖入列表中第一个支持的文件（`isSupportedBookPath` 或 **`.zip` 彩读书包候选**，后者走 `openFilePath` → 书包导入，同「打开文件」）。
...
- **彩读书包解压目录**：设置项 **`bookPackUnpackDir`**（「设置 → 常规」→「书包解压目录」）；**非空**时解压到该绝对路径；**空串**或未配置时回退 **`userData/UnpackedBooks`**（`UNPACKED_BOOKS_DEFAULT_SUBDIR`）。首次启动若设置 JSON 无该键，写入默认路径并持久化。见 **「彩读书包」**。
...
7. **侧栏宽度**  
   非全屏时侧栏仍可拖拽改宽；全屏浮动侧栏宽度仍用同一 `sidebarWidth` 状态（`startResizeSidebar` / `endSidebarResize` 等未改）。多主窗口下其它窗口同步 settings 时 **`loadPersistedSettings({ applySidebarWidth: false })`**，避免拖拽改宽实时同步到其它窗口。
...
- **菜单项**（均受「整体在窗口内」夹取；某条不可用时仍显示为 **disabled**；有当前文件时导出/清除可用）：
  ...
  - **清除阅读数据**（**danger**，`icons.clear`）：确认后清空当前会话路径在 **`file.meta`** 中的进度 / 书签 / 高亮 / 笔记 / 角色卡等，并删除本书立绘缓存目录；保留电子书 **`convertedMdPath`**；写入空壳记录挡住同文件名回退。其下再分隔线。
...
将**当前可读正文**与本书阅读数据打成 ZIP，便于迁移；**不**打包电子书原文件（epub/mobi 等）。实现：`utils/readerBookPack.ts`、`utils/readerBookPackImport.ts`；导出由底栏路径菜单触发，导入走 **`openFilePath`**（打开对话框 / 非侧栏拖放）。

### 包格式

- **建议文件名**：`{书名}-{日期}.colortxt-book.zip`（用户可改名为任意 `.zip`）。
- **结构**：
...
- **入口**：顶栏「打开文件」（过滤器含「彩读书包」/`zip`）、或非侧栏拖放 `.zip`。任意 `.zip` 先作候选；**`parseReaderBookPackZip`** 校验 manifest / `kind`，合法才导入，否则报错中止（不走普通打开）。
- **解包蒙层**：**`bookPackUnpacking`** → `AppOverlays` 全屏「解包中…」（覆盖确认前临时关闭蒙层）。
- **同名匹配顺序**（命中即停）：
...
- **进度**：有 `viewportTopPhysicalLine` 则写入 meta 并清除 `editorViewState`，打开时按该物理行恢复（`openFilePath(..., { restorePhysicalLine })`）。
- **进度落盘注意**（导入后未滚动即退出再启动）：
...
### 各模块单独导出文件名（统一 `colortxt-*`）

| 模块   | 默认文件名                               |
| ------ | ---------------------------------------- |
| 书签   | `{书名}-{日期}.colortxt-bookmarks.json`  |
| 高亮词 | `{书名}-{日期}.colortxt-highlights.json` |
| 笔记   | `{书名}-{日期}.colortxt-notes.json` 或 `.md` |
| 角色卡 | `{书名}-{日期}.colortxt-characters.zip`  |
| 书包   | `{书名}-{日期}.colortxt-book.zip`        |
| AI 对话 | `{书名}-{日期}-{对话标题}[（带思考过程）].colortxt-chat.{json\|md}` |
...
- **导出 / 导入**：侧栏高亮词「更多」→ JSON（`readerHighlightExport.ts`；默认名 **`{书名}-{日期}.colortxt-highlights.json`**）。
- **开关与语法**：
...
- **JSON**：`schemaVersion: 1`，含 `exportedAt`、`bookPath`、`bookName`、`annotations` 数组；默认文件名 **`{书名}-{日期}.colortxt-notes.json`**（见 **「彩读书包」** 文件名表）。
