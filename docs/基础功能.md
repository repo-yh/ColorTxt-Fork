...
- **正文右键**（`AppContextMenu`，Monaco `contextmenu: false`）：**只读**为 **复制**（无选区时禁用）/ **全选**；**编辑态**为剪切 / 复制 / 粘贴（及可用时的 AI 智能排版项）。智能排版 Diff 预览期另有侧栏菜单，不走此项。
...
- **底栏「清空」**：经 **`window.colorTxt.showMessageBox`** 确认后对列表全部路径调用 `clearReadingDataForPaths`；同样不碰会话 / 最近打开 / 文件列表 / UI 设置，不 reload。列表为空时「清空」「删除失效文件」禁用。
