...
- **工具条显示项**（**设置 → 阅读 → 工具条**）：`selectionToolbarButtons`（`copy` / `find` / `askAi` / **`findTarget`**）控制选区工具条按钮显示与「查找」应用目标（查找栏 / 全文搜索，默认查找栏）。主界面设置页以工具条预览 + 左上角复选框，以及与定时滚动「范围」同款的 **`RadioGroup`**；找书设置 **`showFindTargetOption=false`** 隐藏应用目标。
- **浮层钳制**：相对选区水平居中；**优先**钳在阅读区；仅当阅读区钳制后右侧仍会被**窗口**裁切时，水平改按窗口钳制（可伸入侧栏）。垂直始终按阅读区判定上/下展开。
