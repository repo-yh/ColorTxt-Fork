...
- **更多菜单**：侧栏标题行「更多」→ **选择文件**（多选正文/电子书/书包，加入列表不打开，同拖放）→ 分隔线 → **移除失效文件**：对列表全部路径 `file:stat`，`!isFile`（含 ENOENT 不抛错）视为失效，经 `removeFileList` 移出侧栏列表（不删盘、不清阅读数据）；无失效时 toast「没有失效文件」。
...
7. **侧栏宽度**  
   非全屏时侧栏仍可拖拽改宽；全屏浮动侧栏宽度仍用同一 `sidebarWidth` 状态（`startResizeSidebar` / `endSidebarResize` 等未改）。多主窗口下其它窗口同步 settings 时 **`loadPersistedSettings({ applySidebarWidth: false, applyReaderUiPrefs: false })`**，避免拖拽改宽以及阅读/编辑/语音朗读相关 UI 实时同步到其它窗口。
...
- **菜单项**（均受「整体在窗口内」夹取；某条不可用时仍显示为 **disabled**；有当前文件时导出/清除可用）：
  ...
  - **清除阅读数据**（**danger**，`icons.clear`）：确认后调用 `clearReadingDataForPaths`（与设置「阅读数据」面板删除同语义），清空当前会话路径在 **`file.meta`** 中的进度 / 书签 / 高亮 / 笔记 / 角色卡等，并删除本书立绘缓存目录；保留电子书 **`convertedMdPath`**；写入空壳记录挡住同文件名回退。其下再分隔线。
...
- 主题、侧栏展开、语法着色；**压缩空行** / **保留一个空行** / **行首缩进**；**`textConvertZh` / `textConvertLetter` / `textConvertDigit`**（顶栏「转换」，默认 **`off`**）；**`timedScroll`**（定时滚动范围与间隔，见 **`constants/timedScroll.ts`**）；**`readerEditShowLineNumbers`**、**`readerEditMinimap`**、**`editAutoRefreshChapterList`**；章节字数；字号与行高倍数；启动恢复会话；Monaco 高级换行；**滚动**（**`monacoSmoothScrolling`**、**`mouseWheelScrollSensitivity`** 默认 1、**`fastScrollSensitivity`** 默认 5 / 按住 Alt）；**`stickyChapterTitleEnabled`**（启用粘性章节标题，默认 **开启**）等。
...
- **单项删除 / 底栏「删除」**：`clearReadingDataForPaths`——与底栏 **「清除阅读数据」** 同语义（含该书立绘目录）；不关闭当前打开文件；不删会话 / 最近打开 / 文件列表 / 收藏高亮词。
- **「删除失效文件」**：对列表路径调用 **`file:stat`**；**`!isFile`** 视为失效（含磁盘上不存在：主进程对 ENOENT **不抛错**，返回 `isFile`/`isDirectory` 均为 false；亦含目录）。再对其调用 `clearReadingDataForPaths`。
- **底栏「清空」**：确认后对列表全部路径调用 `clearReadingDataForPaths`；同样不碰会话 / 最近打开 / 文件列表 / UI 设置，不 reload。列表为空时「清空」「删除失效文件」禁用。

#### 清除缓存

- **作用**：点 **「清除缓存」**，经 **`window.colorTxt.showMessageBox`** 确认后：
  1. 设置 **`sessionStorage`** **`colorTxt.skipUnloadPersistence`**；
  2. 删除角色立绘缓存**根目录**（`characterPortraitCacheDir`，空则默认路径）；
  3. 从即将保留的 **`colorTxt.ui.settings`** 中去掉 **`highlightWordsByIndexGlobal`**（收藏高亮词）；
  4. `localStorage.clear()` 后写回处理后的 settings；
  5. `window.location.reload()`。
- **会清**：会话、最近打开、文件列表、全部 `file.meta` 阅读数据、收藏高亮词、立绘文件等（以及其它非 settings 的 localStorage 键）。
- **不清**：电子书转换 `.md`、书包解压目录、找书下载目录、**`userData/ai/secrets.v1.json`** 等；界面设置（字号、主题、配色等）保留（除收藏高亮词字段）。
