...
- **章节**：仅识别 ATX 标题（`#` … `######`，行首最多 3 个空白）；`markdownBlockContext` 在围栏代码块与 4 空格/TAB 缩进代码块内跳过 `#`；章节扫描基于**物理行**，避免「行首缩进」展示层误判；侧栏 `headingLevel` 每级缩进 10px；标题行「更多 → 复制目录」按层级空格缩进写入剪贴板（一行一标题）；顶栏「章节匹配规则」对 `.md` 禁用。
- **插图（只读）**：`markdownImages` 用 marked **`Lexer.lexInline`** 解析独占行 `![alt](url)`（与内链同一套，不做非标准兜底）。`readerImageViewZones` 删源行并插 ViewZone；`https:` URL 直链；编辑模式不处理。相对路径经 **`resolveMarkdownAssetAbsPath`** 按 `/` 分段 `joinFs`。
...
7. **侧栏宽度**  
   非全屏时侧栏仍可拖拽改宽；全屏浮动侧栏宽度仍用同一 `sidebarWidth` 状态（`startResizeSidebar` / `endSidebarResize` 等未改）。拖条视觉与 Monaco Diff 分栏 sash 对齐（`--app-sash-*`：4px、`ew-resize`、悬停约 300ms 后显蓝条并渐隐，拖拽中保持高亮）。多主窗口下其它窗口同步 settings 时 **`loadPersistedSettings({ applySidebarWidth: false, applyReaderUiPrefs: false })`**，避免拖拽改宽以及阅读/编辑/语音朗读相关 UI 实时同步到其它窗口。
...
实现集中在 `src/renderer/src/constants/appUi.ts` 与 `src/renderer/src/monaco/readerEditorOptions.ts`（`readerEditorLineHeight` / `letterSpacing`）；行间距另见 `src/renderer/src/monaco/lineSpacing.ts` 与 `electron.vite.config.ts` 对 Monaco `LinesLayout` 的 transform。
...
- **行间距（`lineSpacingPx`）**：默认 `0`（关闭），范围 `0`～`100`（px）。加在每个**物理行**（Monaco model line）最后一条 view 行之后；软换行中间不加距。与「压缩空行时保留一个空行」并存（前者为布局，后者为文本管线）。
- **章节标题前后保留空行（`chapterTitleBlankMode`）**：默认 `before2After1`（前面 2 空行，后面 1 空行）。仅在开启「压缩空行」时生效；可选 `before1`（前 1）、`before1After1`（前 1 后 1）。若开了「保留一个空行」，非标题正文行下方仍会保留一行空行。
...
- **设置面板**：字号、行高、字间距、行间距、左右边距均为滑块；行高滑块的上限随草稿字号变化；拖动字号若导致当前行高超限时，会自动下调行高草稿。
...
由 **`AppFooter.vue`** 渲染，数据与事件由 **`App.vue`** 注入。阅读进度与百分比文案仍来自 **`useAppReadingProgress`**（与其余展示口径一致）。左侧可挂番茄时钟控件（`PomodoroFooterControl`）。
...
- **正文右键**（`AppContextMenu`，Monaco `contextmenu: false`）：**只读**为 **复制**（无选区时禁用）/ **全选** / **编辑选中文本**（`icons.edit`，无选区禁用；选区物理原文超过 `maxPartialEditSelectionChars`（1000）时 **`appAlert`** 提示过大；`textarea` 弹窗改写物理片段并写盘/章节缓存后刷新展示；主界面与找书阅读器均支持；默认快捷键 **`Ctrl+E` / `Command+E`**）；**编辑态**为剪切 / 复制 / 粘贴 / **全选**（及可用时的 **AI 智能排版：选中文本**）。智能排版 Diff 预览期另有侧栏菜单，不走此项。进入/退出编辑模式默认快捷键 **`Ctrl+/` / `Command+/`**（`toggleReaderEdit`）。
...
- 主题、侧栏展开、语法着色；**压缩空行** / **章节标题前后保留空行** / **保留一个空行** / **行首缩进**；**`textConvertZh` / `textConvertLetter` / `textConvertDigit`**（顶栏「转换」，默认 **`off`**）；**`timedScroll`**（定时滚动范围与间隔，见 **`constants/timedScroll.ts`**）；**`readerEditShowLineNumbers`**、**`readerEditMinimap`**、**`editAutoRefreshChapterList`**；章节字数；字号与行高倍数、**`lineSpacingPx`**（行间距，默认 **0**）、**`letterSpacingPx`**（字间距，默认 **0**）；启动恢复会话；Monaco 高级换行；**`monacoCjkWrapOptimize`**（设置 → 阅读「启用中文换行优化」，默认 **开启**：简单换行下将 `“”`/`——`/`……` 等按全角估算并用汉字校准全角字宽；开启高级换行时运行时停用）；**滚动**（**`monacoSmoothScrolling`**、**`mouseWheelScrollSensitivity`** 默认 1、**`fastScrollSensitivity`** 默认 5 / 按住 Alt）；**`stickyChapterTitleEnabled`**（启用粘性章节标题，默认 **开启**）等。
...
- `App.vue` 中对应 `ref` 引用上述常量；`ReaderMain.vue` 的 `withDefaults` 在未由父组件传入时与压缩空行、语法着色、高级换行、中文换行优化、内部行高初值保持一致。已存在本地设置时仍以持久化数据为准。
...
- **导出 / 导入**：侧栏高亮词「更多」→ JSON（`readerHighlightExport.ts`；本书默认名 **`{书名}.highlights.json`**，收藏默认名 **`favorite.highlights.json`**）。
- **开关与语法**：
...
- **动作与默认值**：`src/renderer/src/services/shortcutRegistry.ts` 定义动作 ID、说明、`scope`（`window` 窗口内 / `global` 系统级）及默认 Electron 快捷键字符串。含 **`toggleReaderEdit`**（默认 `Ctrl+/` / `Command+/`）与 **`editSelectedText`**（默认 `Ctrl+E` / `Command+E`，无选区时无操作）。
- **面板区分**：`ShortcutPanel` 的 `panelContext`（`main` / `findBook`）只影响列表展示；绑定仍共用完整 `shortcutBindings`。主窗不展示「书源管理」；找书窗不展示「打开文件 / 选择目录 / 章节匹配规则 / 添加/移除书签」。
...
- **恢复口径（重载当前正文 / 显式物理行）**：切换压缩空行、行首缩进、**转换**、改动「保留一个空行」/「章节标题前后保留空行」、应用章节匹配规则等触发**同路径重开**时，使用 `openFilePath(..., { restorePhysicalLine })`：取**视口末行**经 `viewportDisplayLineToPhysicalLine` 得到物理行，流结束后仍走 `scrollLineToBottom` 显示行对齐（与视图状态恢复互斥）。**仅切换转换**且不重开文件时走 **`applyReaderDisplayFromPhysicalLines`**（见 **「简繁与全半角转换」**）。
