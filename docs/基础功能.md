...
- **列表 / 树状**：**`fileListViewMode`**（`list` | `tree`，默认 `list`）。标题「文件」右侧图标切换；树由当前筛选结果路径去掉公共父目录建成（多盘符多根并列）；文件夹 `title` 为完整目录路径，后缀「（n 文件，大小）」仅统计已加入列表的文件；**按文件大小排序**时同级文件夹按子树列表文件总大小与文件混排；初始化/切换分类时：若当前打开文件在列表中则只展开其路径并定位，否则全部收起；点击文件夹仅展开/折叠，点击文件打开。树状工具栏「分类」左侧：**全部展开 / 全部折叠**（根文件夹皆收起时显示展开，否则显示折叠；展开会打开全部层级）。文件夹右键：批量分类 / 移除（确认）/ 在文件管理器中打开。展开态仅会话内内存，不持久化。
