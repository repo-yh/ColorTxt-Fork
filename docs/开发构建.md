...
│       │   ├── useAppHighlightTerms.ts    # 高亮词展示层与 CRUD/导入导出
...
│       │   ├── lineSpacing.ts            # 物理行后常数行间距（get/set + ViewModel bridge）
│       │   # electron.vite：CJK computeCharWidth/全角样字「汉」；LinesLayout 行间距；optimizeDeps.exclude monaco-editor
...
- **`useAppFileSession.ts`**：打开文件/选目录、会话快照恢复、**彩读书包导入**（`importReaderBookPack` / 侧栏 **`importBookPacksIntoFileList`**）、与流管道和持久化衔接；`resetSession` 置 `readingProgressSynced` 为 `false`；「选择目录」与侧栏拖放会把 `.ctz`/`.ctzx` 与正文分区，书包批量入库（临时密码本、可跳过密码框、默认不打开）；导入目录合并列表时若当前分类筛选为具体分类名，会把新项写上对应 `category`（「全部 / 未分类」筛选下不写）。打开 / 会话恢复时：若仅有 **`viewportTopPhysicalLine`**、无 **`editorViewState`**（如书包带进度导入），仍按物理行恢复视口（见 [基础功能.md](./基础功能.md) → **「彩读书包」**、阅读进度恢复口径）。
...
- **`useAppHighlightTerms.ts`**：本书高亮词展示层刷新、增删/收藏、本书与收藏 JSON 导入导出、侧栏「查找下一处」。
...
- **`appUi.ts`**：UI 常量：存储 key、侧栏宽度、字号/行高/字间距/行间距上下限与步进、`default*` 出厂默认等（无本地设置或与 `persistKey` 字段缺失时；见下文「阅读器字号与行高」「界面与阅读偏好默认值」）；re-export `readerPalette` 的 `applyReaderSurfaceToDocument` 等。
...
| `ReaderSidebar.vue`                                  | 侧栏容器：活动栏含文件 / 章节 / 书签 / 高亮词 / **笔记** / **AI 助手** / **角色** / 搜索（`constants/readerSidebarTab.ts`）。<br>高亮词 tab 为彩色图标；**当前书无高亮词**（或未打开文件）时 **`activityTabBtn--mutedColor`** 灰度显示且加深，与其它 tab 视觉权重接近。<br>挂载 `FileListPanel`、`ChapterListPanel`、`BookmarkListPanel`、`HighlightListPanel`、**`AnnotationListPanel`**、**`AiAssistantPanel`**、**`CharacterSidebarPanel`**、`SearchPanel`。<br>「文件」标题行：**选择目录** + **更多**（选择文件 / 移除失效）；「章节」标题行：**字数**开关 + **更多**；「搜索」标题行：**更多**；其它列表 tab 标题行「更多」锚定各面板菜单。<br>向文件列表下发 **`fileCategory` / `fileSort` / `fileCategoryCatalog`** 并上抛分类相关事件；**标注**导出/导入/清除失效经 **`ReaderSidebar`** 上抛；**`askAiWithQuote`** 切 tab 并 **`prefillQuotedText`**；与 `useReaderSidebarLists`、`useReaderInlineSearch` 等配合；**阅读器编辑**时章节区可提供刷新章节等入口                                                                                                                                                                                                |
| `FileListPanel.vue`                                  | 侧栏「文件」：txt/电子书路径列表、**分类筛选**与 **排序**、编辑模式多选、右键与批量改分类。<br>标题行「更多」：**选择文件**（多选加入列表）/ **移除失效文件**（`file:stat` + `!isFile`，经 `removeFileList`）。<br>编辑模式多选复选框复用全局 **`.checkbox`** 样式（`var(--accent)` 勾选色）。<br>单项右键支持分类/移除/重命名/在新窗口打开/在文件管理器显示（Ctrl+右键附加「清除该文件数据」）；筛选在具体分类时 footer 动作为「清空分类」。<br>`data-drop-zone="file-list"` 标记列表拖放接收区                                                                                                                                                                                                                                                                                                                                           |
...
| `SettingsReadingPanel.vue`                           | 「阅读」：字号/行高滑块、**章节标题前后保留空行**（`chapterTitleBlankMode` 下拉）、压缩空行保留一行、引号/括号跨行匹配、**启用粘性章节标题**、**启用中文换行优化**（`monacoCjkWrapOptimize`）、**滚动**（平滑滚动、**滚动倍率** `mouseWheelScrollSensitivity`、**滚动加速倍率** `fastScrollSensitivity` / 按住 Alt）、全屏正文区宽度、**定时滚动**（范围 **`RadioGroup`**：一屏/一行；间隔 **`NumericInput`** 毫秒）。<br>（`monacoCustomHighlight` 来自 props，用于禁用跨行开关提示）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
