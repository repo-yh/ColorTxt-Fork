...
- **`selectionToolbar.ts`**：选区工具条配置 **`SelectionToolbarButtons`**（`copy` / `find` / **`dictionary`** / `askAi` / **`findTarget`**: `findBar` | `sidebarSearch`）、默认值（复制与问 AI 开、查找与词典关、目标查找栏）与 **`mergeSelectionToolbarButtons`** / **`SELECTION_TOOLBAR_FIND_TARGET_OPTIONS`**；持久化键 **`selectionToolbarButtons`**。
...
- **词典查词**：**`registerDictionaryIpcHandlers`**（`dictionary/registerDictionaryIpc.ts`）注册 **`dictionary:*`**；依赖 **`mdict-js`** 读 `.mdx`（打包前经 **`prune-pack-deps`** 去掉源码/测试目录）；细节见 [基础功能.md](./基础功能.md) → **「词典查词」**。
