...
│       │   ├── cjkWrapOptimize.ts        # 中文换行优化开关（全角标点字宽）
│       │   ├── cjkWrapStrings.ts         # Vite 包装 Monaco strings.isFullWidthCharacter
│       │   ├── lineSpacing.ts            # 物理行后常数段间距（get/set + ViewModel bridge）
│       │   # electron.vite：CJK computeCharWidth/全角样字「汉」；LinesLayout 段间距；optimizeDeps.exclude monaco-editor
...
└── shared/
    ...
    ├── voiceReadEngines.ts       # 引擎注册表（edge / system / winSapi / dashscope / minimax / mimo）
...
- **设置弹窗**：由 **`SettingsPanel.vue`** 组织 **`SettingsTabBar`** 与子面板 **`SettingsGeneralPanel`** / **`SettingsReadingPanel`** / **`SettingsEditPanel`** / **`SettingsAIPanel`** / **`SettingsVectorModelPanel`** / **`SettingsTxt2ImgPanel`**（页签文案「角色卡」，文生图与角色卡出图配置）/ **`SettingsSkillsPanel`** / **`SettingsVoiceReadPanel`**（**语音朗读**）/ **`SettingsWebDavPanel`**（**WebDAV**）；技能编辑用 **`SettingsSkillEditModal.vue`**（见下文组件表）。
...
- **`useReaderSidebarLists.ts`**：侧栏文件/章节/书签虚拟列表、过滤与滚动同步；文件列表按 **`fileCategory`** 筛选、按 **`fileSort`** 排序，与项上 `category` / `addedAt` 等字段合并展示。**打开时间** / **阅读进度**排序切入时拍快照，避免打开文件或滚动更新进度时重排。章节列表视口联动滚动受 **`suppressChapterListAutoScroll`** 抑制（进/出编辑、切换压缩空行等）；须在 **`syncChaptersAfterViewportSettled`** 的 `finally` 或流错误路径中恢复，否则换章不再居中当前章。
...
- **`useReaderAnnotations.ts`**：阅读器**划线 / 笔记**状态机：选区浮动工具条（`ReaderSelectionToolbar`）、笔记输入面板（`ReaderNoteInputPanel`）、标注 hit 索引与视口 inline 装饰（`readerAnnotationDecor.ts`）；`emitUpsert` / `emitRemove` 上抛至 `App.vue` 写 **`readerAnnotations`**。**点击已有标注**时绑定 draft 而不改选区；**移除划线**（色盘前「移除」）且无笔记时移除整条记录。工具条 **「查找」** 经 **`emitFindWithQuote`**：按 **`findTarget`** 打开 Monaco 查找栏（**`openFindWithSearchString`**）或上抛 **`searchWithQuote`** 填入侧栏全文搜索（同时收起工具条）。**问 AI** 填入侧栏引用后不关闭工具条（`suppressToolbarUntilMs`）。浮层几何：**优先**按阅读区钳制；仅当钳完后右侧仍会被**窗口**裁切时，水平改按窗口钳制（可伸入侧栏）。色盘 **`colorIndex`** 按当前标注色列表长度解析，越界回退**最后一色**（`clampLineationColorIndex`）。**编辑模式**下不建 hit 索引、不挂 Monaco 装饰并关闭工具条；**只读**下 **`rebuildAnnotationIndex`** 经 **`buildAnnotationHitsByDisplayLine`** 按物理列映射展示列后同步视口装饰。侧栏/导出引用原文统一走 **`resolveAnnotationDisplayQuote`**（见 **「阅读器标注与笔记」**）。
...
- **`selectionToolbar.ts`**：选区工具条配置 **`SelectionToolbarButtons`**（`copy` / `find` / `askAi` / **`findTarget`**: `findBar` | `sidebarSearch`）、默认值（复制与问 AI 开、查找关、目标查找栏）与 **`mergeSelectionToolbarButtons`** / **`SELECTION_TOOLBAR_FIND_TARGET_OPTIONS`**；持久化键 **`selectionToolbarButtons`**。
...
- **`readerHighlightGeometry.ts`**：自定义高亮词浮动层（`ReaderHighlightFloat`）与**选区标注工具条**（`ReaderSelectionToolbar`）的几何与布局计算（`computeFloatPlacement` 按传入 clip 钳制；阅读区优先 / 窗口补救的策略在 **`useReaderAnnotations.applyFloatPlacement`**）。
...
- **`fileMetaStore.ts`**：单文件元数据：书签、**`readerAnnotations`**、**`highlightWordsByIndex`**（词组 **`string[][]`**，**`normalizeHighlightWordsByIndex`** 兼容旧扁平表）、末行/进度等；与 `colorTxt.file.meta` 同步。
...
- **`readerAnnotationExport.ts`**：标注 **JSON**（`schemaVersion: 1`）与 **Markdown** 导出/导入；默认文件名 `{书名}.notes.md|json`；Markdown 按章节 `##` 分组、笔记 **`💡`** + 原文 blockquote、纯划线 **`✨`**、文末 `*导出于 …*`；导出原文经可选 **`resolveQuoteText`**（默认 **`ann.text`**，运行时由 `App.vue` 注入 **`resolveAnnotationDisplayQuote`**）。
...
| `ChapterListPanel.vue`                               | 侧栏「章节」：章节列表、字数开关、跳转当前章。<br>标题行「更多」：**复制目录**（按 `headingLevel` 缩进空格，一行一标题，写入剪贴板）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `ColorSchemePanel.vue`                               | 配色弹窗容器：`ColorSchemeTabBar` + 上述三面板；可用 **`visibleTabs`** 限制标签（找书仅 `['reader']`，仍显示 TabBar）。<br>确定时按可见标签 emit **`applyReaderPalettes`** / **`applyHighlightColors`** / **`applyLineationColors`**；打开时从 props 同步草稿                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `SettingsReadingPanel.vue`                           | 「阅读」：字号/行间距滑块、**章节标题上下保留空行**（`chapterTitleBlankMode` 下拉）、压缩空行保留一行、引号/括号跨行匹配、**启用粘性章节标题**、**启用中文换行优化**（`monacoCjkWrapOptimize`）、**滚动**（平滑滚动、**滚动倍率** `mouseWheelScrollSensitivity`、**滚动加速倍率** `fastScrollSensitivity` / 按住 Alt）、全屏正文区宽度、**定时滚动**（范围 **`RadioGroup`**：一屏/一行；间隔 **`NumericInput`** 毫秒）。<br>（`monacoCustomHighlight` 来自 props，用于禁用跨行开关提示）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `SettingsVoiceReadPanel.vue`                         | 「语音朗读」：独立 **朗读方案** 区块（最多 12 套，含引擎、单/多音色、密钥）；**引擎**下拉（Edge TTS / 系统语音 / **讲述人自然语音（Windows `winSapi`）** / 通义 / MiniMax / **小米 MiMo**）；**朗读方案**（单音色 / 旁白·对白多音色）；多音色下 **对白引号样式**、**AI 识别**（需 AI 总开关）、**情绪标注**开关（AI 识别开启且引擎支持时；关闭则不向 TTS 传情绪）；MiMo 含 **VoiceDesign**（声音描述、智能润色）、**VoiceClone**（参考音频）；通义 / MiniMax / MiMo **API 密钥** + **`AppConnectionTestButton`**；`winSapi` 说明后以 warning 提示「当前不可用 / 未检测到音色」；模型建议（**`ApiEndpointInput`**）；语速/音调；试听预览（切 tab / 关设置时 **`cancelPreview`**）。**密钥与 AI 对话/文生图分开存** |
...
| `ReaderSelectionToolbar.vue`                         | 只读选区浮动工具条：复制 / 查找 / 问 AI 受 **`selectionToolbarButtons`**（设置 → 阅读 → 工具条）控制；高亮词需 `monacoCustomHighlight`；马克笔 / 波浪线 / 直线（色块取自当前主题 **`lineationColors`**；已有划线时色盘前显示「移除」）；**记笔记**（已有笔记时 **`hasNote` 激活态**；有划线时笔记图标跟标注色）；**问 AI** 另需 AI 总开关。几何由 **`readerHighlightGeometry.ts`** + **`useReaderAnnotations.applyFloatPlacement`**（优先阅读区钳制，右侧会被窗口裁时再按窗口水平钳制）计算                                                                                                                                                                                                                                                                                                                                                                                      |
| `SettingsSelectionToolbarPreview.vue`                | 设置 → 阅读 →「工具条」预览：外观同选区工具条；「复制」「查找」「问 AI」左上角复选框写入 **`selectionToolbarButtons`** |
