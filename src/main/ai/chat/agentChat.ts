...
import { runCharacterPortraitExtract } from "../tools/characterPortrait";
import { runMindmapTool } from "../tools/mindmapTool";
import { runWordcloudTool } from "../tools/wordcloudTool";
import {
...
function buildAgentSystemPrompt(
  ...
  if (ragEnabled && chapterMatchRuleOnlyTurn) {
    ...
    lines.push(
      ...
      "- 用户提到「本章」「这章」「这一章」「当前章」「该章」，或要求**总结 / 概括 / 梳理 / 讲了什么 / 说了什么**当前阅读进度下的这一章且索引 ≥0 时：**必须先**调用 ragContext，参数 chapterIndex **等于**该索引（从 0 起），以加载该章正文；**禁止**仅用 ragSearch 代替（检索片段可能来自其它章或不完整）。",
...
function formatReadingAnchorForTurn(
  ...
  if (ragEnabled) {
    return (
      ...
      `用户刚发起本轮提问时的阅读位置以上为准；若对话历史中仍有其它章节的摘要或工具结果，**不得**据此回答本轮「本章 / 这章 / 这一章 / 当前章 / 该章 / 总结本章」类问题，必须重新调用 ragContext(${idx}) 后再作答。`
    );
  }
  return (
