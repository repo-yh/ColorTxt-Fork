...
export function buildAgentToolsWithSkills(
  ...
  // 依赖向量索引（RAG）的工具：向量检索关闭时排除；其余工具（getFullText 等）不依赖 RAG，始终开放
